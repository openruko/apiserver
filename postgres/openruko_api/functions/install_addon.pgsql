...
BEGIN
  ...
  SELECT a.id, p.id FROM addon a, addon_plan p
   WHERE a.name = v_addon_name and p.name = v_addon_plan
  ...
  SELECT EXISTS(
    SELECT id FROM app_addon
     WHERE app_id = p_app_id AND addon_id = v_addon_id AND plan_id = v_addon_plan_id)
  ...
  SELECT array_agg(installed_addons.name) FROM
    (SELECT a.name || ':' || p.name as name
      ...
      WHERE aa.app_id = p_app_id AND aa.addon_id = a.id AND aa.plan_id = p.id) AS installed_addons
    INTO v_new_addons;
